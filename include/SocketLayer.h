...
public:
	...
	int SendTo( SOCKET s, const char *data, int length, char ip[ 16 ], unsigned short port, RakPeer* rakPeer );
	...
	/// \param[in] port The port number to send to.
	/// \return 0 on success, nonzero on failure.
	int SendTo( SOCKET s, const char *data, int length, unsigned int binaryAddress, unsigned short port, RakPeer* rakPeer );
