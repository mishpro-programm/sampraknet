...
//		return;
	...
	SocketLayer::Instance()->SendTo( connectionSocket, (const char*)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), ( char* ) host, remotePort, this );
...
SOCKET RakPeer::GetConnectionSocket(){
...
#endif
{
	...
	if (rakPeer->IsBanned( rakPeer->PlayerIDToDottedIP( playerId ) ))
	{
		...
		SocketLayer::Instance()->SendTo( rakPeer->connectionSocket, (char*)&c, 2, playerId.binaryAddress, playerId.port, rakPeer );
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
	...
	else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_COOKIE && length == sizeof(unsigned char)*2+1) // 0.3.7 RAKSAMP HACK PONPON
	{
		...
		*(WORD*)&c[1] = (*(WORD*)&data[1] ^ NETCODE_CONNCOOKIELULZ);
		...
		SocketLayer::Instance()->SendTo( rakPeer->connectionSocket, (char*)&c, 3, playerId.binaryAddress, playerId.port, rakPeer );
		return;
	}
	...
	else
	{
			...
			|| (unsigned char)(data)[0] == ID_PING)	&& length == sizeof(unsigned char)+sizeof(RakNetTime) )
		{
				...
				rakPeer->AllowIncomingConnections() ) // Open connections with players
			{
				...
				SocketLayer::Instance()->SendTo( rakPeer->connectionSocket, (const char*)outBitStream.GetData(), outBitStream.GetNumberOfBytesUsed(), (char*)rakPeer->PlayerIDToDottedIP(playerId) , playerId.port, rakPeer );
#endif
			}
		}
...
//	unsigned numberOfBytesUsed;
	...
	while (rcs)
	{
		...
		if (rcs->nextRequestTime < timeMS)
		{
			...
			SocketLayer::Instance()->SendTo( connectionSocket, (char*)&c, 3, rcs->playerId.binaryAddress, rcs->playerId.port, this );
		}
	...
	//	remoteSystemList[ remoteSystemIndex ].allowPlayerIdAssigment=true;
		if ( remoteSystemList[ remoteSystemIndex ].isActive )
		{
			...
			remoteSystem->reliabilityLayer.Update( connectionSocket, playerId, MTUSize, timeNS, messageHandlerList, this ); // playerId only used for the internet simulator test
