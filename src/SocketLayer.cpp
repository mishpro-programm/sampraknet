...
int SocketLayer::RecvFrom( const SOCKET s, RakPeer *rakPeer, int *errorCode )
{
	...
	bool isClient = rakPeer->IsClient();
	...
	if ( len != SOCKET_ERROR )
	{
		...
		if(!isClient) {
			...
			int port = GetLocalPort(rakPeer->GetConnectionSocket());

			unKyretardizeDatagram((unsigned char *)data, len, port, 0);
...
#endif
int SocketLayer::SendTo( SOCKET s, const char *data, int length, unsigned int binaryAddress, unsigned short port, RakPeer* rakPeer )
{
	if ( s == INVALID_SOCKET )
	{
		return -1;
...
int SocketLayer::SendTo( SOCKET s, const char *data, int length, char ip[ 16 ], unsigned short port, RakPeer* rakPeer )
{
	unsigned int binaryAddress;
	binaryAddress = inet_addr( ip );
	return SendTo( s, data, length, binaryAddress, port, rakPeer );
}
